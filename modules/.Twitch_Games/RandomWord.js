...
module.exports = {
  ...
  main: async (TWITCHBOT, room, user, message) => {
    ...
    let res = {
      type: 'action',
      msg: `${user.username} is quick! GivePLZ ${winnerPoints}HP for typing ${settings.word} first!`
    };
    ...
    switch (msgA[0]) {
      case 'word':
        if (!msgA[1]) {
          ...
          BotResponse(TWITCHBOT, settings.editors[0], user.username, {
            type: 'action',
            msg: `PogChamp Word Time! quickly type ${settings.word.toUpperCase()} for ${winnerPoints}HP! PogChamp`
          });
        }
        break;
      case 'enabled':
      ...
      case 'pointsmin':
        if (parseInt(msgA[1]) > 0) {
          ...
          res.msg = `Random Word POINT MIN has been set to: ${settings.pointsMin}`
        } else {
      ...
      case 'pointsmax':
        if (parseInt(msgA[1]) > 0) {
          ...
          res.msg = `Random Word POINT MAX has been set to: ${settings.pointsMax}`
        } else {
